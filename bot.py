...
def generate_content(message):
    ...
    try:
        ...
        # Отправляем invoice сразу после базового ответа
        title = "Премиум сценарий"
        description = f"Полный сценарий + визуалы + монетизация для темы: {text}"
        invoice_payload = f"premium_{text[:20].replace(' ', '_')}"
        currency = "XTR"
        ...
        bot.send_invoice(
            ...
            invoice_payload=invoice_payload,  # правильное имя параметра
            provider_token="",
            currency=currency,
            prices=prices,
